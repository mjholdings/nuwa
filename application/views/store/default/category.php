				...
				<div class="sidebar-block">
					...
					function display_with_children($parentRow, $level = 0) {
						...
						foreach ($parentRow as $key => $row) {
							...
							echo '<li data-id="' . $row['id'] . '" class="' . ($row['children'] ? 'has-children' : '') . '" ><span>' . $space . '<a href="' . base_url('store/category/' . $row['slug']) . $url_location . '">' . $row['name'] . "</a></span>" . ($row['children'] ? "<i class='fa fa-angle-down icon-category'></i>" : "");
							if ($row['children']) {
					...
					echo '<li data-id="0" ><span><a href="' . base_url('store/category/') . $url_location . '">' . __('store.all_categories') . '</a></span>';
