...
//                }
    ...
    public
        ...
        if ($object->title) {
            ...
            foreach ($products as $product) {
                ...
                $product_stock = $this->db->get_where(
                    'product_meta',
                    array(
                ...
                foreach ($products as $key => $product) {
                    ...
                    if ($product['refer_id'] > 0 && !in_array($product['refer_id'], $restricted_vendors)) {
                        ...
                        if ($product['commission'] > 0) {
                            $comission_data_batch[] = array(
                                ...
                                'comment' => 'Hoa hồng cho Đơn hàng Id order_id=' . $order_id . ' | Xếp Theo : ' . $order_info['firstname'] . " " . $order_info['lastname'] . " <br> Sale done from ip_message",
                    ...
                    if ($product['admin_commission'] > 0 && $vendors['admin_sale_status'] == 1) {
                        $comission_data_batch[] = array(
                            ...
                            'comment' => 'Admin Hoa hồng cho Đơn hàng Id order_id=' . $order_id . ' | Xếp Theo : ' . $order_info['firstname'] . " " . $order_info['lastname'] . " <br> Sale done from ip_message",
                    ...
                    if ($product['refer_id'] > 0 && !in_array($product['refer_id'], $restricted_vendors)) {
                        if ($refer_status) {
                            ...
                            for ($l = 1; $l <= $max_level; $l++) {
                                ...
                                if ($s && $levelUser > 0 && !in_array($levelUser, $restricted_vendors) && ($l <= 3 || ($l > 3 && $arrTotalUser['sale_localstore_total_revenue'] > 2000000))) {
                                    if ($setting['sale_type'] == 'percentage') {
                                        if (strpos($product["commission_type"], "percentage") >= 0) {
                                            $total = $product["total"] - $product["commission"];
                                        } else {
                                            $total = $product["total"];
                                        }
                                    ...
                                    if ($_giveAmount > 0) {
                                        $comission_data_batch[] = array(
                                            ...
                                            'user_id' => $levelUser,
                                            'amount' => $_giveAmount,
                                            'comment' => "Level {$l} : " . 'Hoa hồng cho Đơn hàng Id order_id=' . $order_id . ' | Thành viên : ' . $order_info['firstname'] . " " . $order_info['lastname'],
                    ...
                    if ($product['vendor_commission'] > 0 && !in_array($product['vendor_id'], $restricted_vendors)) {
                        $comission_data_batch[] = array(
                            ...
                            'comment' => 'Vendor Bán hàng Nhận Hòa hồng cho đơn Id order_id=' . $order_id . ' | Xếp Theo : ' . $order_info['firstname'] . " " . $order_info['lastname'] . " <br> Sale done from ip_message",
    ...
    public
        ...
        $notificationData = array(
            ...
            'notification_title' => 'Đơn hàng Mới đã phát sinh bởi ' . $userDetail['username'],
    ...
    public
        ...
        if (isset($_SESSION['guestFlowClassified']))
            unset($_SESSION['guestFlowClassified']);
        $uncompletedData = $this->Product_model->getByField('uncompleted_payment', 'id', $uncompleted_id);
