...
class Total_model extends MY_Model
{
	...
	public function adminBalanceWallet($wallet = 'reward', $filter = [])
	{
		$wallet_where = ' 1 ';
		...
		// amount for refer sale comission wallet
		$balance += $this->db->query("SELECT SUM(amount) as total FROM wallet WHERE {$wallet_where} AND user_id > 1 AND amount > 0
		AND status != 0 AND wallet_to = '{$wallet}' AND is_vendor = 0 AND withdraw_request = 0 ")->row()->total;
	...
	public function adminTotals()
	{
		...
		// Tiêu dùng nạp
		$totals['admin_balance_purchase'] = $this->adminBalanceWallet('purchase');

		// Tài khoản rút
		$totals['admin_balance_withdraw'] = $this->adminBalanceWallet('withdraw');
	...
	public function getUserTotals($user_id)
	{
		...
		$sale_localstore_total_query = $this->db->query("SELECT SUM(total) as total, SUM(commission) as total_commission,COUNT(*) as total_order FROM order_products WHERE refer_id={$user_id} GROUP BY order_id ")->result();
		...
		$sale_localstore_total_revenue_query = $this->db->query("SELECT SUM(order_products.total) as total FROM order_products INNER JOIN `order` ON order_products.order_id = `order`.`id` WHERE `order`.`status` = 1 AND order_products.refer_id={$user_id} GROUP BY order_products.order_id ")->result();

		foreach ($sale_localstore_total_revenue_query as $key => $value) {
			$totals['sale_localstore_total_revenue'] += $value->total;
		}
	...
	public function vendor_user_totals_week_growth($user_id, $filter = [], $current)
	{
		...
		$welcome_bonus = $this->db->query("SELECT SUM(amount) as total FROM wallet WHERE type IN('welcome_bonus','membership_plan_bonus','reward') AND user_id={$user_id} {$wallet_where} ")->row()->total;

		$wallet_requests = $this->db->query("SELECT SUM(amount) as total FROM wallet WHERE withdraw_request = 1 AND status != 1  AND user_id={$user_id} {$wallet_where} ")->row()->total;
	...
	public function chartUser($userid, $filter = [])
	{
		...
		if ($filter['group'] == 'day') {
			$groupby = 'CONCAT(DAY(created_at),"-",MONTH(created_at),"-",YEAR(created_at))';
			$groupbyInteg = 'CONCAT(DAY(ica.created_at),"-",MONTH(ica.created_at),"-",YEAR(ica.created_at))';
		} else if ($filter['group'] == 'week') {
			$groupby = 'WEEK(created_at)';
			$groupbyInteg = 'WEEK(ica.created_at)';
		} else if ($filter['group'] == 'month') {
			$groupby = 'MONTH(created_at)';
			$groupbyInteg = 'MONTH(ica.created_at)';
		} else if ($filter['group'] == 'year') {
			$groupby = 'YEAR(created_at)';
			$groupbyInteg = 'YEAR(ica.created_at)';
		}
