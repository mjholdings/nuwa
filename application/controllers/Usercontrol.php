...
class Usercontrol extends MY_Controller
{
			...
			if ($this->userdetails()) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					if (!isset($json['errors'])) {
						...
						if ((int)$commition_setting['status'] == 0) {
							$refid  = 0;
						} else if ((int)$commition_setting['status'] == 2 && in_array($refid, $disabled_for)) {
							$refid = 0;
						}
	...
	public function dashboard()
	{
		...
		$data['populer_users'] = $this->Product_model->getPopulerUsers(array("limit" => 10, "current_user_id" => $userdetails['id']));
		...
		foreach ($data['reward'] as $reward) {
			if ($data['user_totals']['wallet_unpaid_amount'] == $reward['minimum_earning']) {
				$amount = ($reward['minimum_earning'] * $reward['sale_comission_rate']) / 100;
				$this->load->model('Wallet_model');
				$this->Wallet_model->addTransaction(array(
					'status'       => 1,
					'user_id'      => $userdetails['id'],
					'amount'       => (float)$amount,
					'comment'      => 'Reward ' . $reward['name'],
					'type'         => 'reward',
					'comm_from'    => 'reward',
					'reference_id' => 0,
		...
		$data['user_reward'] = json_decode($userdetails['user_reward'], true);

		$data['branch_reward'] = json_decode($userdetails['branch_reward'], true);
	...
	public function my_network()
	{
		...
		if ((int)$referlevelSettings['status'] == 0) {
			show_404();
		} else if ((int)$referlevelSettings['status'] == 2 && in_array($userdetails['id'], $disabled_for)) {
			$refer_status = false;
		}
		...
		else if ($referlevel_setting['show_sponser'] == '') {
	...
	public function get_withdrawal_modal()
	{
		...
		$amount_request = $this->input->post("amount", true);
	...
	public function confirm_deposit()
	{
		...
		if ($vendorDepositStatus['depositstatus']) {
			...
			if (!isset($userdetails['id']) || empty($userdetails['id'])) {
			...
			} else if ($data['payment_gateway'] == 'paystack' && !in_array(
				...
				$vendor_deposit['vd_amount'] = $this->currency->convert(
					$data['amount'],
					$this->session->userdata('userCurrency'),
