	...
	public function user_plan_modal()
	{
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			if (!isset($json['errors'])) {
				...
				$membership = $plan->buy($user, $post['status_id'], $post['comment'], 'Đặc cách bởi Admin', 0, '', $plan);
				...
				// MJ CẬP NHẬT LẠI THƯỞNG NẾU ĐẶC CÁCH THAY ĐỔI VỊ TRÍ ==========
	...
	public function assignDefaultPlans($membership)
	{
		switch ((int)$membership['status']) {
			case 1:
				...
				break;
			case 2:
				$users = $this->db->query('SELECT id, is_vendor FROM users WHERE (plan_id=-1 or plan_id=0) and type="user" and status=1 and reg_approved=1 and is_vendor=1')->result_array();
				break;
			case 3:
