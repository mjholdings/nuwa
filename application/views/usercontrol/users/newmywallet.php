...
<div class="row mb-4">
	<div class="<?= ($userdetails['is_vendor'] == 1) ? "col-xl-3" : "col-xl-3"; ?>">
		<div class="card border mt-2">
			<div class="card-header bg-primary">
				<h6 class='card-title text-center text-uppercase text-white m-0'><?= __('user.balance') ?></h6>
			</div>
			<div class="card-body">
	...
	<div class="<?= ($userdetails['is_vendor'] == 1) ? "col-xl-3" : "col-xl-3"; ?>">
		<div class="card border mt-2">
			<div class="card-header bg-danger">
				<h6 class='card-title text-center text-uppercase text-white m-0'><?= __('Ví Thưởng') ?></h6>
			</div>
			<div class="card-body">
				<div class="text-center">
					<ul class="list-inline row mb-0 clearfix">
	...
	<div class="<?= ($userdetails['is_vendor'] == 1) ? "col-xl-3" : "col-xl-3"; ?>">
		<div class="card border mt-2">
			<div class="card-header bg-secondary">
				<h6 class='card-title text-center text-uppercase text-white m-0'><?= __('Ví Điểm') ?></h6>
			</div>
			<div class="card-body">
				<div class="text-center">
					<ul class="list-inline row mb-0 clearfix">
	...
	<?php if ($userdetails['is_vendor'] == 1) { ?>
		<div class="col-xl-3">
			<div class="card border mt-2">
				<div class="card-header bg-success">
					<h6 class='card-title text-center text-uppercase text-white m-0'><?= __('user.total_sales') ?></h6>
				</div>
				<div class="card-body">
					<div class="text-center">
						<ul class="list-inline row mb-0 clearfix">
	...
	<?php } ?>
	<div class="<?= ($userdetails['is_vendor'] == 1) ? "col-xl-3" : "col-xl-2"; ?> d-none">
		<div class="card border mt-2">
											...
											<div class="modal-dialog">
												...
												<div class="modal-content">
													<div class="modal-header">
														<h4 class="modal-title"><?= __('Yêu cầu rút/chuyển tiền') ?></h4>
...
<script type="text/javascript">
	...
	$('.withdrawal-unpaid').on('click', function() {
		...
		if (amount > dataVal) {
			alert('Quý khách chỉ được rút 70% số tiền trong ví');
		} else {
	...
	function withdrawal_payments(ids, _this, amount) {
		if (amount == '0') {
			...
			$.ajax({
				url: '<?= base_url("usercontrol/get_withdrawal_modal_request") ?>',
				type: 'POST',
				dataType: 'json',
				data: {
					ids: ids,
	...
	$('.send-request').on('click', function() {
		...
		$.ajax({
			...
			dataType: 'json',
			data: {
				request_payment: $this.attr("data-id")
			},
			beforeSend: function() {
				$this.btn("loading");
			},
	...
	$("#modal-confirm .modal-body").delegate("[delete-tran-confirm]", "click", function() {
		...
		$.ajax({
			...
			dataType: 'json',
			data: {
				id: $this.attr("delete-tran-confirm")
			},
			beforeSend: function() {
				$this.button("loading");
			},
