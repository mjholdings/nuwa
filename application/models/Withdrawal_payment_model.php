...
class Withdrawal_payment_model extends CI_Model
{
	...
	public $status_list = array(
		...
		'9'  =>  'Đảo ngược',
		'10' =>  'Đã hủy bỏ',
		'11' =>  'Hủy không được thực hiện',
		'12'  =>  'Đang chờ thanh toán',
		'13'  =>  'Chưa giải quyết',
	);
	...
	public function apiAddWithdrwalRequest($code, $ids, $total, $setting = array())
	{
		...
		if ($request['total'] > 0) {
			...
			$this->db->query("INSERT INTO wallet SET  user_id={$request['user_id']},type='admin_wallet_requests', amount={$total}, status=0, withdraw_request=1, created_at='{$date}'");

			$insert_ids = $this->db->insert_id();

			$this->db->query("UPDATE wallet_requests SET tran_ids={$insert_ids}");

			$this->db->query("INSERT INTO wallet_requests_history SET created_at='{$date}', req_id={$insert_id}, status=0, comment='Yêu cầu đã được tiếp nhận'");
